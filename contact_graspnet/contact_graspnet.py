...
def get_bin_vals(global_config):
    ...
    if not global_config["TEST"]["allow_zero_margin"]:
        bin_vals = np.minimum(
            bin_vals,
            global_config["DATA"]["gripper_width"]
            - global_config["TEST"]["extra_opening"],
        )
...
def get_model(point_cloud, is_training, global_config, bn_decay=None):
    ...
    if (
        "raw_num_points" in data_config
        and data_config["raw_num_points"] != data_config["ndataset_points"]
    ):
    ...
    if joint_heads:
        ...
        head = tf_util.dropout(
            head, keep_prob=0.7, is_training=is_training, scope="dp1"
        )
        head = tf_util.conv1d(
            head, 4, 1, padding="VALID", activation_fn=None, scope="fc2"
        )
    ...
    end_points["grasp_offset_pred"] = (
        tf.math.sigmoid(grasp_offset_head)
        if model_config["bin_offsets"]
        else grasp_offset_head
    )
...
def build_6d_grasp(
    approach_dirs, base_dirs, contact_pts, thickness, use_tf=False, gripper_depth=0.1034
):  # 0.1034
    """
    ...
    if use_tf:
        grasps_R = tf.stack(
            [base_dirs, tf.linalg.cross(approach_dirs, base_dirs), approach_dirs],
            axis=3,
        )
        grasps_t = (
            contact_pts
        ...
        grasps = tf.concat(
            [tf.concat([grasps_R, tf.expand_dims(grasps_t, 3)], axis=3), homog_vec],
            axis=2,
        )
    else:
        grasps = []
...
def get_losses(
    ...
    min_geom_loss_divisor = (
        tf.constant(float(global_config["LOSS"]["min_geom_loss_divisor"]))
        if "min_geom_loss_divisor" in global_config["LOSS"]
    ...
    pred_grasps = build_6d_grasp(
        approach_dir_head, grasp_dir_head, pointclouds_pl, thickness_pred, use_tf=True
    )  # b x num_point x 4 x 4
    gt_grasps_proj = build_6d_grasp(
        approach_labels_pc_cam,
        dir_labels_pc_cam,
        pointclouds_pl,
        thickness_gt,
    ...
    pos_gt_grasps_proj = tf.where(
        tf.broadcast_to(
            tf.expand_dims(
                tf.expand_dims(tf.cast(grasp_success_labels_pc, tf.bool), 2), 3
            ),
    ...
    gripper_control_points_homog = tf.concat(
        [
            ...
            tf.ones(
                (
                    global_config["OPTIMIZER"]["batch_size"],
                    gripper_control_points.shape[1],
                    1,
                )
            ),
        ],
    ...
    control_points = tf.keras.backend.repeat_elements(
        tf.expand_dims(gripper_control_points_homog, 1), gt_grasps_proj.shape[1], axis=1
    )  # b x num_point x 5 x 4
    ...
    pred_control_points = tf.matmul(
        control_points, tf.transpose(pred_grasps, perm=[0, 1, 3, 2])
    )[
        :, :, :, :3
    ]  #  b x num_point x 5 x 3

    ### Pred Grasp to GT Grasp ADD-S Loss
    gt_control_points = tf.matmul(
        control_points, tf.transpose(pos_gt_grasps_proj, perm=[0, 1, 3, 2])
    )[
        :, :, :, :3
    ]  #  b x num_pos_grasp_point x 5 x 3
    sym_gt_control_points = tf.matmul(
        sym_control_points, tf.transpose(pos_gt_grasps_proj, perm=[0, 1, 3, 2])
    )[
        :, :, :, :3
    ]  #  b x num_pos_grasp_point x 5 x 3
    ...
    squared_add = tf.reduce_sum(
        ...
        ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    sym_squared_add = tf.reduce_sum(
        (
            tf.expand_dims(pred_control_points, 2)
            - tf.expand_dims(sym_gt_control_points, 1)
        )
        ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    ...
    neg_squared_adds = -tf.concat(
        [squared_add, sym_squared_add], axis=2
    ...
    gt_control_points = tf.matmul(
        control_points, tf.transpose(gt_grasps_proj, perm=[0, 1, 3, 2])
    )[
        :, :, :, :3
    ]  #  b x num_pos_grasp_point x 5 x 3
    sym_gt_control_points = tf.matmul(
        sym_control_points, tf.transpose(gt_grasps_proj, perm=[0, 1, 3, 2])
    )[
        :, :, :, :3
    ]  #  b x num_pos_grasp_point x 5 x 3

    neg_squared_adds = -tf.reduce_sum(
        (tf.expand_dims(pred_control_points, 1) - tf.expand_dims(gt_control_points, 2))
        ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    neg_squared_adds_sym = -tf.reduce_sum(
        (
            ...
            - tf.expand_dims(sym_gt_control_points, 2)
        )
        ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    ...
    neg_squared_adds_k_gt2pred, pred_grasp_idcs = tf.math.top_k(
        neg_squared_adds, k=1, sorted=False
    )  # b x num_pos_grasp_point
    neg_squared_adds_k_sym_gt2pred, pred_grasp_sym_idcs = tf.math.top_k(
        neg_squared_adds_sym, k=1, sorted=False
    )  # b x num_pos_grasp_point
    ...
    batch_idcs = tf.meshgrid(
        tf.range(pred_grasp_idcs_joined.shape[1]),
    ...
    adds_loss_gt2pred = tf.reduce_mean(
        tf.reduce_sum(nearest_pred_grasp_confidence * masked_min_adds_gt2pred, axis=1)
        / pos_grasps_in_view
    )

    ### Grasp baseline Loss
    cosine_distance = tf.constant(1.0) - tf.reduce_sum(
    ...
    approach_labels_orthog = tf.math.l2_normalize(
        approach_labels_pc_cam
        - tf.reduce_sum(
            tf.multiply(grasp_dir_head, approach_labels_pc_cam), axis=2, keepdims=True
        )
        * grasp_dir_head,
        axis=2,
    )
    cosine_distance_approach = tf.constant(1.0) - tf.reduce_sum(
    ...
    if global_config["MODEL"]["bin_offsets"]:
        if global_config["LOSS"]["offset_loss_type"] == "softmax_cross_entropy":
            ...
            if (
                "too_small_offset_pred_bin_factor" in global_config["LOSS"]
                and global_config["LOSS"]["too_small_offset_pred_bin_factor"]
            ):
                ...
                ) * too_small_offset_pred_bin_factor + tf.constant(1.0)
                offset_loss = tf.multiply(collision_weight, offset_loss)

            offset_loss = tf.reduce_mean(
                tf.multiply(tf.reshape(tf_bin_weights, (1, 1, -1)), offset_loss), axis=2
            )
    else:
    ...
    if (
        "topk_confidence" in global_config["LOSS"]
        and global_config["LOSS"]["topk_confidence"]
    ):
        bin_ce_loss, _ = tf.math.top_k(
            tf.squeeze(bin_ce_loss), k=global_config["LOSS"]["topk_confidence"]
        )
...
def multi_bin_labels(cont_labels, bin_boundaries):
    ...
    for b in range(len(bin_boundaries) - 1):
        bins.append(
            tf.math.logical_and(
                tf.greater_equal(cont_labels, bin_boundaries[b]),
                tf.less(cont_labels, bin_boundaries[b + 1]),
            )
        )
...
def compute_labels(
    pos_contact_pts_mesh,
    pos_contact_dirs_mesh,
    pos_contact_approaches_mesh,
    pos_finger_diffs,
    pc_cam_pl,
    camera_pose_pl,
    global_config,
    ...
    squared_dists_all = tf.reduce_sum(
        (tf.expand_dims(contact_point_batch_cam, 1) - tf.expand_dims(xyz_cam, 2)) ** 2,
        axis=3,
    )
    ...
    grasp_success_labels_pc = tf.cast(
        tf.less(tf.reduce_mean(squared_dists_k, axis=2), radius * radius), tf.float32
    ...
    dir_labels_pc_cam = tf.math.l2_normalize(
        tf.reduce_mean(grouped_dirs_pc_cam, axis=2), axis=2
    )  # (batch_size, num_point, 3)
    approach_labels_pc_cam = tf.math.l2_normalize(
        tf.reduce_mean(grouped_approaches_pc_cam, axis=2), axis=2
    )  # (batch_size, num_point, 3)
